...
M3 (+Text)     & 1980 & $-$0.9 & 0.762 & 0.281 \\
M4 (+Satellite)& 1989 & $-$0.4 & 0.595 & 0.852 \\
M5 (Full)      & 1869 & $-$6.4 & 0.060 & 0.195 \\
\bottomrule
\end{tabular}
\end{table}

The key finding is that \emph{no single alternative data category} provides a statistically significant improvement in isolation (M1--M4 all have DM~$p > 0.05$), but the \emph{combination} of all categories (M5) delivers a~6.4\% RMSE reduction that is marginally significant under the DM test ($p = 0.060$) and directionally consistent under the GW test ($p = 0.195$).  The block-bootstrap 95\% confidence interval for the M5 improvement is $[-13.0, +1.0]$\%, spanning zero but concentrated on the negative side.
...
{Category} & {Share (\%)} \\
\midrule
Baseline (conventional macro) & 59.2 \\
C: Text / Sentiment            & 19.0 \\
A: Trade activity               &  8.5 \\
B: Financial                    &  7.4 \\
D: Satellite / Geospatial      &  5.8 \\
\bottomrule
\end{tabular}
\end{table}

Conventional macro predictors remain the backbone (59.2\% of total SHAP importance), but alternative data collectively account for~40.8\%.  Within alternative data, the text/sentiment category dominates (19.0\%)---driven primarily by the ECB FinBERT sentiment and BdF Business Climate Indicator---followed by trade-activity proxies (8.5\%), financial indicators (7.4\%), and satellite data (5.8\%).  Satellite data contribute modestly overall but show spikes during supply-chain disruption episodes (COVID port closures, Suez Canal blockage).
...
The dominance of text/sentiment data (19.0\% SHAP share) is striking.  Two mechanisms explain this result:
\begin{enumerate}
...
The full model's DM $p$-value of~0.060 falls short of the conventional 5\% threshold.  However, three considerations support the economic significance of the result.  First, the 6.4\% RMSE improvement is directionally consistent and economically meaningful: applied to France's average quarterly goods trade balance (EUR~4.5~billion), this translates to a reduction in forecast error of approximately EUR~129~million.  Second, the pseudo-real-time vintage exercise confirms that M5 outperforms M0 in \emph{every one} of 50 Monte Carlo replications, indicating that the improvement is robust and not a chance outcome.  Third, using the observed effect size ($d = 0.064$, $\sigma_d = 0.046$) and a one-sided test at 5\%, achieving power of~80\% would require approximately $T \approx 170$ monthly evaluation observations---roughly 14~years of data---compared to our 108 months.  The marginal significance is thus largely attributable to the short sample rather than a weak signal.
...
This chapter demonstrates that alternative data---trade-activity proxies, financial-flow indicators, NLP-based sentiment, and satellite proxies---collectively provide a~6.4\% improvement in BoP nowcast accuracy beyond what conventional macroeconomic predictors alone can achieve (DM $p = 0.060$).  No single category dominates, but their combination exploits complementary information that Ridge regression efficiently aggregates.  Text/sentiment data emerge as the highest-value category (19.0\% SHAP share), driven by the forward-looking nature of central bank communication and survey indicators.  The \citet{giacomini2010forecast} fluctuation test reveals that the M5 advantage is concentrated during crisis episodes, while the \citet{rossi2013advances} stability diagnostics confirm that the gain is \emph{persistent and stable} over the full evaluation window (sup-DM $p = 0.70$; CUSUM~$= 0.77 < 1.36$).  A Monte Carlo power analysis shows that the MCS retaining all models is expected given the low empirical power ($30.6$\%) at our sample size, and a pseudo-real-time vintage exercise confirms robustness to data revision noise (100\% of $50$ replications favour M5).
