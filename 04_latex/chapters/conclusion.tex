...
\textbf{Chapter~1} conducted a systematic horse race between ten models spanning autoregressive benchmarks, econometric approaches (DFM, Bridge), regularised linear ML (Ridge, LASSO), nonlinear ML (Gradient Boosting, XGBoost), and deep learning (LSTM, GRU).  In a pseudo-real-time expanding-window evaluation on quarterly data (2000--2022), \textbf{Ridge regression achieved the largest improvement: a~26.3\% RMSE reduction over AR(1)} ($p = 0.013$).  Traditional econometric models delivered moderate gains (DFM~$-18.9\%$, Bridge~$-13.8\%$), nonlinear ML improved on the benchmark but was dominated by Ridge, and both LSTM ($+1.0\%$) and GRU ($+1.0\%$) failed entirely.  The key takeaway: in small-sample macroeconomic settings, the bias--variance tradeoff favours regularisation over complexity.

\textbf{Chapter~2} conducted a structured ablation study to quantify the incremental value of four categories of alternative data: trade-activity proxies, financial-flow indicators, text/sentiment data, and satellite/geospatial proxies.  The \textbf{full model yielded a~6.5\% RMSE reduction} over baseline Ridge (DM $p = 0.059$), with \textbf{text/sentiment data providing the largest SHAP contribution (19.8\%)}.  No individual category was significant in isolation---but their combination, optimally aggregated by Ridge's $\ell_2$ penalty, was marginally significant at the~10\% level.  All data sources come from real public APIs, ensuring full reproducibility.

\textbf{Chapter~3} bridged accuracy and policy relevance through three exercises.  Crisis evaluation showed Ridge maintained a~17\% RMSE reduction during COVID-19, precisely when timely information was most valuable.  Cross-country tests confirmed generalisability: \textbf{16--20\% RMSE reductions for France, Germany, and Italy} (all $p < 0.01$), while Spain's improvement ($-2.5$\%) was not significant.  A BoP-augmented Taylor rule with interest-rate smoothing yielded a long-run current account gap coefficient of~$-0.46$; the \textbf{maximum gap between nowcast-implied and official-data-implied policy rates reached~2.7 basis points} at the monthly level, concentrated during crisis episodes.
...
\begin{itemize}
  ...
  \item \textbf{Deep learning implementation.}  The failure of both LSTM and GRU may partly reflect our implementation choices (single-layer LSTM and two-layer GRU, each with 32 hidden units).  More sophisticated architectures---Temporal Fusion Transformers \citep{lim2021temporal} or pre-trained foundation models---might perform better, though sample-size constraints remain binding.
