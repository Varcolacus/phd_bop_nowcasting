...
\textbf{Chapter~2} conducted a structured ablation study to quantify the incremental value of four categories of alternative data: trade-activity proxies, financial-flow indicators, text/sentiment data, and satellite/geospatial proxies.  The \textbf{full model yielded a~6.4\% RMSE reduction} over baseline Ridge (DM $p = 0.060$), with \textbf{text/sentiment data providing the largest SHAP contribution (19.0\%)}.  No individual category was significant in isolation---but their combination, optimally aggregated by Ridge's $\ell_2$ penalty, was marginally significant at the~10\% level.  All data sources come from real public APIs, ensuring full reproducibility.

\textbf{Chapter~3} bridged accuracy and policy relevance through three exercises.  Crisis evaluation showed Ridge maintained a~17\% RMSE reduction during COVID-19, precisely when timely information was most valuable.  Cross-country tests confirmed generalisability: \textbf{16--20\% RMSE reductions for France, Germany, and Italy} (all $p < 0.01$), while Spain's improvement ($-2.5$\%) was not significant.  A BoP-augmented Taylor rule with interest-rate smoothing yielded a long-run current account gap coefficient of~$-0.46$; the \textbf{maximum gap between nowcast-implied and official-data-implied policy rates reached~2.6 basis points} at the monthly level, concentrated during crisis episodes.
