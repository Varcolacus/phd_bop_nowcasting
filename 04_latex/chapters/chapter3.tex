...
The Hodrick--Prescott (HP) filter used to construct the output and CA gaps has been criticised by \citet{hamilton2018why} for generating spurious cyclical dynamics.  As a robustness check, we re-extract the output gap using the \citet{hamilton2018why} regression filter---regressing $y_{t+h}$ on $(y_t, y_{t-1}, y_{t-2}, y_{t-3})$ with $h = 8$ for monthly data---and re-estimate the Taylor rule with this alternative output gap measure.  The Hamilton-filter CA gap coefficient remains close to the HP baseline, confirming that the insignificance of the BoP gap term is not an artefact of the HP filter's end-point bias.
